import { PlayerStore, AudioState } from '../../stores/player.store';
...
export class PlaybackManager {
	...
	constructor(private playerStore: PlayerStore, private settingsStore: SettingsStore) {
		...
		combineLatest(
			...
			.subscribe(() => {
				...
				const seek = seconds / playerStore.state.currentTrackDuration;
				playerStore.setCurrentTrackSeeker(seek * 100);
			});
	...
	private onTrackLoaded() {
		...
		this.playerStore.setCurrentTrackSeeker(0);
	 ...
	 */
	private onSeeked() {
		console.log('on sought');
		this.playerStore.setCurrentTrackSeeker(this.audio.seek());
